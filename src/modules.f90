...
implicit none
  ...
  character(len=500) :: original_file
...
subroutine read_parfile(rank)
  ...
  integer :: pos,icount,i,interval,rank
...
!  endif
  ...
  do while (ios == 0)
     ...
     if (ios == 0) then
        ...
        case ('input_rewrite:')
           read(buffer, *, iostat=ios) input_rewrite
!        case ('DC_OBS:')
...
subroutine read_OBSfile(iOBS,rank)
  ...
  if(NumOBS.gt.1) then
        write(OBS_num,*)iOBS
        file_name2 = trim(adjustl(folder_input)) // trim(adjustl(su_file0))
        file_name=trim(adjustl(file_name2)) // trim(adjustl(OBS_num))
  endif

  original_file=file_name

  INQUIRE(file=original_file,EXIST=su_exist)
  if(.NOT. su_exist)     then
       write(*,*)'WARNING: SU Field data: ',trim(adjustl(original_file)),' not found'
        if(rank.eq.0)call ascii_art(2)
